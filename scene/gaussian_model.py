...
class GaussianModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp_U = P.orthogonal(nn.Linear(feat_dim, feat_dim, bias = False))
        self.mlp_U = self.mlp_U.cuda()
        self.mlp_V = P.orthogonal(nn.Linear(feat_dim, feat_dim, bias = False))
        self.mlp_V = self.mlp_V.cuda()
        self.Gain = torch.nn.Parameter(torch.ones(1, self.feat_dim, device='cuda'), requires_grad=True)
    ...
    def training_setup(self, training_args):
        ...
        if self.use_feat_bank:
            l = [
                ...
                {'params': self.mlp_U.parameters(), 'lr': training_args.mlp_U_lr_init, "name": "mlp_U"},
                {'params': self.mlp_V.parameters(), 'lr': training_args.mlp_V_lr_init, "name": "mlp_V"},
                {'params': self.Gain, 'lr': training_args.gain_lr_init, "name": "gain"},
            ]
        else:
            l = [
                {'params': [self._anchor], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "anchor"},
        ...
        self.mlp_U_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_U_lr_init,
                                                    lr_final=training_args.mlp_U_lr_final,
                                                    lr_delay_mult=training_args.mlp_U_lr_delay_mult,
                                                    max_steps=training_args.mlp_U_lr_max_steps)
        
        self.mlp_V_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_V_lr_init,
                                                    lr_final=training_args.mlp_V_lr_final,
                                                    lr_delay_mult=training_args.mlp_V_lr_delay_mult,
                                                    max_steps=training_args.mlp_V_lr_max_steps)
        
        self.gain_scheduler_args = get_expon_lr_func(lr_init=training_args.gain_lr_init,
                                                    lr_final=training_args.gain_lr_final,
                                                    lr_delay_mult=training_args.gain_lr_delay_mult,
                                                    max_steps=training_args.gain_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_U":
                lr = self.mlp_U_scheduler_args(iteration)
                param_group['lr'] = lr

            if param_group["name"] == "mlp_V":
                lr = self.mlp_V_scheduler_args(iteration)
                param_group['lr'] = lr

            if param_group["name"] == "gain":
    ...
    def load_mlp_checkpoints(self,path):
        ...
        self.mlp_U.load_state_dict(checkpoint['mlp_U'])
        self.mlp_V.load_state_dict(checkpoint['mlp_V'])
        
        self.Gain = torch.nn.Parameter(checkpoint['gain'].to(self.Gain.device))
    ...
    def estimate_final_bits(self):
        ...
        #   step 1: mean removal
        _feat = _feat - mean
        #   step 2: (note: remember the order)
        _feat = self.mlp_V(_feat)
        _feat = self.Gain * _feat
        _feat = self.mlp_U(_feat)
        ...
        bit_feat = self.entropy_gaussian.forward(_feat, torch.zeros_like(_feat), scale, Q_feat)
    ...
    def conduct_encoding(self, pre_path_name):
        ...
        for s in range(steps):
            ...
            #feat_residual_name = os.path.join(pre_path_name, 'f_residual.pth').replace('.pth', f'_{s}.pth')
            #scaling_residual_name = os.path.join(pre_path_name, 's_residual.pth').replace('.pth', f'_{s}.pth')
            ...
            #feat = _feat[N_start:N_end].view(-1) - mean  # [N_num*32]
            feat = _feat[N_start:N_end] - mean

            feat = self.mlp_V(feat)
            feat = self.Gain * feat
            feat = self.mlp_U(feat)
            
            feat = STE_multistep.apply(feat.contiguous().view(-1), Q_feat)
            
            torch.cuda.synchronize(); t0 = time.time()
            bit_feat = encoder_gaussian_chunk(feat, torch.zeros_like(feat), scale, Q_feat, file_name=feat_b_name)
    ...
    def conduct_decoding(self, pre_path_name, patched_infos):
        ...
        for s in range(steps):
            ...
            feat_decoded = decoder_gaussian_chunk(torch.zeros_like(scale), scale, Q_feat, file_name=feat_b_name)
            #feat_decoded = feat_decoded + mean
            feat_decoded = feat_decoded.view(N_num, self.feat_dim)  # [N_num, 32]
            
            U = self.mlp_U.weight
            feat_decoded = torch.matmul(feat_decoded, U)
            feat_decoded = feat_decoded / self.Gain
            V = self.mlp_V.weight
            feat_decoded = torch.matmul(feat_decoded, V)
            feat_decoded = feat_decoded + mean
